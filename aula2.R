...
rating.Histrogram <- ggplot(data = dados) + geom_histogram(mapping = aes(x = Rating),na.rm = T,breaks = seq(1,5,1)) ## parametro na.rm ir?? excluir todos os valores NaN, assim, ir?? remover o WARNING gerado anteriormente
rating.Histrogram
...
category.Plot <- ggplot(data = dados) + geom_bar(mapping = aes(x = Category),stat = "count")
category.Plot

# # simulando gr??fico ruim horizontal
category.Plot.Horiz <- ggplot(data = dados) + geom_bar(mapping = aes(x = Category) ,stat = "count") + coord_flip()
category.Plot.Horiz
