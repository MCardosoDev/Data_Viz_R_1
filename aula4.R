...
data.frame(data_hora) %>% mutate(m = month(data_hora,label = T))
...
notas <- read.csv(file = "C:/DS/DataViz1/user_reviews.csv")
str(notas)
...
nota_plot <- ggplot(media_nota) + geom_line(aes(x = as.Date(data_2), y = media)) 
